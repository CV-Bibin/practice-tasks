...
export default function CreateSearch20Task() {
  ...
  const handleContextChange = (e) =>
    setTaskContext({ ...taskContext, [e.target.name]: e.target.value });
    ...
    setResults(
      results.map((r) => {
        if (r.id === id) {
          const newErrors = {
            ...r.addrErrors,
            [errorField]: !r.addrErrors[errorField],
          };
  ...
  const addResult = () => {
    if (results.length < 5)
      setResults([
        ...results,
        { ...initialResultState, id: results.length + 1 },
      ]);
  };
      ...
      <div style={styles.leftScrollColumn}>
        <div style={styles.headerBlock}>
          ...
          <p style={styles.helpText}>
            Configure the viewport, define user location, and add results. The
            map is pinned to the right.
          </p>
        </div>
        ...
        <div style={styles.sectionCard}>
          ...
          <div style={styles.grid2Col}>
            <div>
              ...
              <input
                ...
                style={styles.input}
                name="query"
                value={taskContext.query}
                onChange={handleContextChange}
                placeholder="e.g. Starbucks near me"
              />
            </div>
            <div>
            ...
            <div>
              ...
              <input
                ...
                style={styles.input}
                name="viewportCoords"
                value={taskContext.viewportCoords}
                onChange={handleContextChange}
                placeholder="e.g. 10.2605, 76.9270"
              />
            </div>
            <div>
          ...
          <label style={{ ...styles.label, marginTop: "16px" }}>
            Viewport Bounding Box Size (Offset)
          </label>
          <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
            <input
              ...
              step="0.001"
              name="viewportSize"
              value={taskContext.viewportSize}
              onChange={handleContextChange}
              style={{ flex: 1 }}
            />
            <span style={{ fontSize: "13px", fontWeight: "bold" }}>
              {taskContext.viewportSize}
            </span>
          </div>
        </div>
        ...
        <div style={styles.sectionCard}>
          ...
          >
            <h3 style={{ ...styles.subHeading, margin: 0 }}>
              2. Result Data & Gold Answers ({results.length}/5)
            </h3>
          </div>
            ...
            <div style={{ display: "flex", gap: "16px", marginTop: "8px" }}>
              <label style={styles.radioLabel}>
                <input
                  ...
                  name="isNavigational"
                  value="Yes"
                  checked={taskContext.isNavigational === "Yes"}
                  onChange={handleContextChange}
                />{" "}
                Yes
              </label>
              <label style={styles.radioLabel}>
                <input
                  ...
                  name="isNavigational"
                  value="No"
                  checked={taskContext.isNavigational === "No"}
                  onChange={handleContextChange}
                />{" "}
                No
              </label>
          ...
          {results.map((res, index) => (
            <div
              ...
              style={{
                ...styles.resultCard,
                borderLeft: `6px solid ${pinColors[index]}`,
              }}
            >
              <div
                style={{
              ...
              >
                <strong style={{ color: pinColors[index], fontSize: "16px" }}>
                  Result #{res.id}
                </strong>
                {results.length > 1 && (
                  <button
              ...
              >
                <div>
                  ...
                  <input
                    ...
                    value={res.name}
                    onChange={(e) =>
                      handleResultChange(res.id, "name", e.target.value)
                    }
                  />
                </div>
                <div>
                  ...
                  <textarea
                    ...
                    value={res.address}
                    onChange={(e) =>
                      handleResultChange(res.id, "address", e.target.value)
                    }
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    value={res.coords}
                    onChange={(e) =>
                      handleResultChange(res.id, "coords", e.target.value)
                    }
                  />
                </div>
              </div>
              ...
              <label style={styles.checkboxItem}>
                <input
                  type="checkbox"
                  checked={res.unexpectedLanguage}
                  onChange={() =>
                    handleCheckboxToggle(res.id, "unexpectedLanguage")
                  }
                />{" "}
              ...
              >
                <input
                  type="checkbox"
                  checked={res.poiClosed}
                  onChange={() => handleCheckboxToggle(res.id, "poiClosed")}
                />{" "}
              ...
              <div style={styles.formGroup}>
                ...
                <select
                  ...
                  value={res.relevance}
                  onChange={(e) =>
                    handleResultChange(res.id, "relevance", e.target.value)
                  }
                >
                ...
                {["Good", "Acceptable", "Poor", "Bad"].includes(
                  res.relevance,
                ) && (
                  <div style={styles.indentedBlock}>
                    <label style={styles.checkboxItem}>
                      <input
                        type="checkbox"
                        checked={res.relUserIntent}
                        onChange={() =>
                          handleCheckboxToggle(res.id, "relUserIntent")
                        }
                      />{" "}
                      User intent issue
                    </label>
                    <label style={styles.checkboxItem}>
                      <input
                        type="checkbox"
                        checked={res.relDistance}
                        onChange={() =>
                          handleCheckboxToggle(res.id, "relDistance")
                        }
                      />{" "}
              ...
              <div
                style={{
                  ...styles.formGroup,
                  opacity: res.poiClosed ? 0.5 : 1,
                }}
              >
                <label style={styles.label}>Name and Category Accuracy</label>
                <select
                  required={!res.poiClosed}
                  disabled={res.poiClosed}
                  style={styles.select}
                  value={res.nameAcc}
                  onChange={(e) =>
                    handleResultChange(res.id, "nameAcc", e.target.value)
                  }
                >
                  <option value="" disabled hidden>
                    Select...
                ...
                {(res.nameAcc === "Incorrect" ||
                  res.nameAcc === "Partially Correct") &&
                  !res.poiClosed && (
                    <div style={styles.indentedBlock}>
                      <label style={styles.checkboxItem}>
                        <input
                          type="checkbox"
                          checked={res.nameIssue}
                          onChange={() =>
                            handleCheckboxToggle(res.id, "nameIssue")
                          }
                        />{" "}
                        Name Issue
                      </label>
                      <label style={styles.checkboxItem}>
                        <input
                          type="checkbox"
                          checked={res.categoryIssue}
                          onChange={() =>
                            handleCheckboxToggle(res.id, "categoryIssue")
                          }
                        />{" "}
                        Category Issue
                      </label>
              ...
              >
                ...
                <select
                  ...
                  disabled={res.poiClosed}
                  style={styles.select}
                  value={res.addressAcc}
                  onChange={(e) =>
                    handleResultChange(res.id, "addressAcc", e.target.value)
                  }
                >
                  <option value="" disabled hidden>
                    Select...
                ...
                {res.addressAcc === "Incorrect" && !res.poiClosed && (
                  ...
                  >
                    {[
                      ...
                      { key: "subLoc", label: "Sub-Locality" },
                      { key: "loc", label: "Locality" },
                      { key: "region", label: "Region/State" },
                      { key: "postal", label: "Postal Code" },
                      { key: "country", label: "Country" },
                      { key: "notExist", label: "Address does not exist" },
                      { key: "lang", label: "Language/Script issue" },
                      {
                        key: "countrySpecific",
                        label: "Country specific issue",
                      },
                      ...
                      <label key={error.key} style={styles.checkboxItem}>
                        <input
                          type="checkbox"
                          checked={res.addrErrors[error.key]}
                          onChange={() =>
                            handleAddrErrorToggle(res.id, error.key)
                          }
                        />{" "}
                        {error.label}
                      </label>
              ...
              >
                ...
                <select
                  ...
                  value={res.pinAcc}
                  onChange={(e) =>
                    handleResultChange(res.id, "pinAcc", e.target.value)
                  }
                >
                  <option value="" disabled hidden>
                    Select...
          ...
          {results.length < 5 && (
            <button type="button" onClick={addResult} style={styles.addBtn}>
              + Add Another Result
            </button>
          )}
      ...
      <div style={styles.rightStickyColumn}>
        <div style={styles.stickyMapWrapper}>
          ...
          <div style={{ flex: 1, position: "relative" }}>
            <TaskMapPreview
              userCoords={taskContext.userCoords}
              viewportCoords={taskContext.viewportCoords}
              viewportSize={taskContext.viewportSize}
              results={results}
            />
          </div>
        </div>
